...
Click anywhere (or press Space) to begin a fresh walk — the title screen also offers *continue the walk* (resume a saved run), a language toggle, a volume slider, and a reminder to play with sound on: this is a game you hear.
...
| **⟲** (top-right button) | Start over from anywhere (with confirmation) |
...
- **Explore** the enclosed canyon: rolling dune swells, boulders, broken pillars, obelisks, and great arches form alcoves where instruments hide. A short action-gated tutorial and a golden guide marker lead you to the first voice.
...
- Sing along the dune crests to ride a sand-slide. Runs connect, too: the title screen greets the last songkeeper by the song they left behind, and a faint ghost companion may retrace that past wanderer's steps.
- **Collect all 10** and the altar calls you: a conducted finale where the ten voices approach one at a time — each singing its own phrase, a gold label naming who calls — and your sung note is the baton (a mistimed note just circles once and returns; the song cannot fail). Then a victory card names the song your journey created.
- **Keep wandering** after the song ends, or sing into the finished canyon and the sanctuary answers in the order you taught it. Lore stones re-tell their chapters as you pass, and *begin anew* starts a song that will be different from every one before it.
...
- Conducted finale: the ten released voices loop their phrases bar after bar around the altar while the voice in flight sings its own over them; after your final note the canyon falls silent — but sing, and it still answers

## Tech

- `index.html` — everything in one file (~3400 lines)
- HTML5 Canvas 2D
- Web Audio API
